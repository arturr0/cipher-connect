...
document.addEventListener('DOMContentLoaded', () => {
	...
	async function loadImageAsync(src, timeout = 5000) {
		const img = new Image();
		img.src = src;
		...
		const loadPromise = new Promise((resolve, reject) => {
			img.onload = async () => {
				try {
					if (!img.complete) {
						// If the image is not fully loaded, decode it manually.
						await img.decode();
					}
					resolve(img);
				} catch (error) {
		...
		const timeoutPromise = new Promise((_, reject) =>
			setTimeout(() => reject(new Error(`Image load timed out: ${src}`)), timeout)
		);
			...
			sendButton.addEventListener('click', async () => {
				...
				socket.once('foundUsers', (foundUsers) => {
					...
					if (foundUser) {
						...
						receiverElement.textContent = receiver;						const profileContainer = userDiv.querySelector('.profile-container');
