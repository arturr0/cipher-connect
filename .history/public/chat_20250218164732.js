...
document.addEventListener('DOMContentLoaded', () => {
	...
	socket.on('foundUsers', async (founded) => {
		...
		founded.forEach((user) => {
			const userDiv = document.createElement('div');
			userDiv.classList.add('user');
			userDiv.style.display = 'none'; // Keep hidden until all images are loaded
		...
		// Preload all images before making divs visible
		await Promise.all(imageLoadPromises);
		
		// Show all users once images are loaded
	...
	async function updateSearchedProfileImage(container, imageSrc, initials) {
		...
		try {
			...
			console.error(error.message);
			initials.style.display = 'flex'; // Show initials if image fails
		}
	}
