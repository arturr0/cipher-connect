...
document.addEventListener('DOMContentLoaded', () => {
	...
	socket.on('foundUsers', async (founded) => {
		...
		for (const user of founded) {
			const userDiv = document.createElement('div');
			userDiv.classList.add('user');
			userDiv.style.display = 'none'; // Keep hidden until images load
			...
			// Load and set profile image before showing the user
			await updateSearchedProfileImage(profileContainer, user.profileImage, initials);
		...
		// Now that all images are loaded, reveal users
