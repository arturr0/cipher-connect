...
dropdowns.forEach(dropdown => {
    // Use touchstart event to toggle dropdown visibility
    dropdown.addEventListener("touchstart", function (event) {
        event.stopPropagation(); // Prevent event from bubbling up to the document

        let content = this.querySelector(".dropdown-content");

        // Toggle active state to show/hide the dropdown content
        content.classList.toggle("active");
    });
});

// Close dropdown when touching or clicking outside
document.addEventListener("touchstart", function (event) {
    dropdowns.forEach(dropdown => {
        let content = dropdown.querySelector(".dropdown-content");
