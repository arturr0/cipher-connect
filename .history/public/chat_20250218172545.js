...
function loadProfileImage(container, imageSrc, initials, userDiv) {
    ...
    // Low-quality image placeholder (small size, blurred)
    const lowQualityImage = `${imageSrc}?q=10`; // Assume the server can serve lower quality images by adding query params
    img.src = lowQualityImage; // Initially load a low-quality image

    // Once the image is loaded, replace with the high-quality image
    img.onload = () => {
        img.src = imageSrc; // Replace the low-quality image with the original once it is loaded
        userDiv.style.visibility = 'visible'; // Make user div visible once image is loaded
        userDiv.classList.add('visible');
    };

    // Use IntersectionObserver to load the image when it’s in view
