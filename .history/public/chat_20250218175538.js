...
document.addEventListener('DOMContentLoaded', () => {
	...
	socket.on('foundUsers', async (founded) => {
		...
		founded.forEach((user) => {
			...
			// Skip users that have the 'friends' class
			if (user.isFriend === 1) {
				userDiv.classList.add('friends'); // Add friends class if it's a friend
			}
			...
			// Load profile image with IntersectionObserver for lazy loading, but only for non-friend users
			if (!userDiv.classList.contains('friends')) {
				loadProfileImage(profileContainer, user.profileImage, initials, userDiv);
			} else {
				// Directly load the image for friends (no lazy-loading)
				const img = new Image();
				img.classList.add('searchedAvatar');
				img.src = user.profileImage;
				img.onload = () => {
					userDiv.style.visibility = 'visible'; // Make the user div visible after loading the image
	...
	function loadProfileImage(container, imageSrc, initials, userDiv) {
		...
		image.classList.add('searchedAvatar');
