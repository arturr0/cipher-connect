...
socket.on('foundUsers', async (founded) => {
    ...
    // For each user, create an element and append it
    founded.forEach((user) => {
        const userDiv = document.createElement('div');
        userDiv.classList.add('user');
        userDiv.classList.add('lazy-load'); // For lazy load styling
        ...
        // Lazy loading the profile image using IntersectionObserver
...
// Function to lazy-load the profile image when it becomes visible in the viewport
function loadProfileImage(container, imageSrc, initials, userDiv) {
    if (!imageSrc) {
        initials.style.display = 'flex';  // Show initials if image is not provided
        userDiv.style.visibility = 'visible'; // Make user div visible immediately
        userDiv.classList.add('visible'); // Add visible class for lazy-load
        return;
    }
    ...
    img.style.height = '40px';
    img.style.borderRadius = '50%';
    img.loading = 'lazy'; // Enable lazy loading for better performance on large images

    // Set a small 1x1 placeholder image for initial display
    img.src = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/wcAAwAB/FP8bKkAAAAASUVORK5CYII=';

    // Set up srcset to serve smaller images for mobile or low-resolution networks
    img.srcset = `${imageSrc}?w=40 1x, ${imageSrc}?w=80 2x`; // Set resolution for different device resolutions

    // Use IntersectionObserver to only load the image when it’s in view
    const observer = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                img.src = imageSrc; // Load the real image once it’s in view
    ...
    img.onload = () => {
        ...
        userDiv.style.visibility = 'visible'; // Make user div visible once image is loaded
        userDiv.classList.add('visible');
        initials.style.display = 'none'; // Hide initials once the image is loaded
    };

    img.onerror = () => {
        // Handle error if the image fails to load
        console.error(`Failed to load image: ${imageSrc}`);
        initials.style.display = 'flex'; // Show initials if image fails to load
    };

    // Append image to the profile container
    container.appendChild(img);
}
