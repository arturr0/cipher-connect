...
document.addEventListener('DOMContentLoaded', () => {
	...
	socket.on('foundUsers', async (founded) => {
		...
		// Create a fragment to append elements all at once
		const fragment = document.createDocumentFragment();
		
		// Array to store promises for image loading
		const imageLoadPromises = [];
	
		// Loop through users and create user divs
		founded.forEach((user) => {
			const userDiv = document.createElement('div');
			userDiv.classList.add('user');
			userDiv.style.display = 'none'; // Keep hidden initially (does not take space)
			...
			// Load profile image
			imageLoadPromises.push(loadImage(profileContainer, user.profileImage, initials, userDiv));
		});
	
		usersDiv.appendChild(fragment);
	
		// Wait for all images to load before displaying user divs
		await Promise.all(imageLoadPromises);
	
		// After all images are loaded, reveal the divs
		document.querySelectorAll('.user').forEach(user => {
			user.style.display = 'flex'; // Make the user div visible and part of layout
			user.style.visibility = 'visible'; // Ensure it's fully visible
		});
	});
	
	// Function to load the image asynchronously and handle fallback to initials
	async function loadImage(container, imageSrc, initials, userDiv) {
		if (!imageSrc) {
			initials.style.display = 'flex'; // Show initials if no image
			return;
		}
		...
		try {
			const img = await loadsearchedImageAsync(imageSrc);
			img.style.width = '40px';
			img.style.height = '40px';
			img.style.borderRadius = '50%';
			img.classList.add('userAvatar');
			container.appendChild(img); // Append image to container
			initials.style.display = 'none'; // Hide initials once image is loaded
			userDiv.style.visibility = 'visible'; // Make the user div visible once image is loaded
		} catch (error) {
			console.error(error.message);
			initials.style.display = 'flex'; // Fallback to initials if image fails to load
			userDiv.style.visibility = 'visible'; // Ensure div is visible if image fails
		}
	}
	
	// Function to load the image asynchronously
	async function loadsearchedImageAsync(src) {
		return new Promise((resolve, reject) => {
			const img = new Image();
			img.src = src;
