...
socket.on('foundUsers', async (founded) => {
    ...
    founded.forEach((user) => {
        const userDiv = document.createElement('div');
        userDiv.classList.add('user');
        userDiv.style.visibility = 'hidden'; // Make the user div initially hidden
        ...
        imageLoadPromises.push(loadProfileImage(profileContainer, user.profileImage, initials));

    });

    users.appendChild(fragment);

    // Wait for all images to load before showing .user divs
    await Promise.all(imageLoadPromises);

    // Now show the users after images are loaded
...
async function loadProfileImage(container, imageSrc, initials) {
    if (!imageSrc) {
        initials.style.display = 'flex';
        return;
    }
    ...
    try {
        ...
        img.setAttribute('data-src', imageSrc); // Use data-src initially

        // Create an IntersectionObserver to load the image when it's about to come into view
        const observer = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    img.src = img.getAttribute('data-src'); // Set src when in view
