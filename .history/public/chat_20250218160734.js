...
document.addEventListener('DOMContentLoaded', () => {
	...
	socket.on('foundUsers', async (founded) => {
		...
		const usersArray = [];
		const imagePromises = [];
		const imagesMap = new Map(); // Store loaded images
	
		// Preload images before rendering anything
		for (const user of founded) {
			if (!user.profileImage) {
				console.warn(`No profile image for ${user.username}, using initials.`);
				imagesMap.set(user.username, null); // Ensure initials are used
				continue;
			}
	
			const imagePromise = loadsearchedImageAsync(user.profileImage)
				.then((img) => {
					imagesMap.set(user.username, img); // Store successfully loaded image
				})
				.catch((error) => {
					console.warn(`Image failed to load for ${user.username}:`, error);
					imagesMap.set(user.username, null); // Store null for failed images
				});
	
			imagePromises.push(imagePromise);
		}
	
		// Wait for images but still render users with initials if needed
		await Promise.allSettled(imagePromises);
	
		for (const user of founded) {
			const userDiv = document.createElement('div');
			userDiv.classList.add('user');
			usersArray.push(userDiv);
			...
			// Get preloaded image
			const img = imagesMap.get(user.username);
			if (img) {
				img.style.width = '40px';
				img.style.height = '40px';
				img.style.borderRadius = '50%';
				...
				initials.style.display = 'none'; // Hide initials if image loaded
			} else {
				console.warn(`No image available for ${user.username}, using initials.`);
				initials.style.display = 'flex'; // Show initials if image failed
			}
		...
		// Append everything at once (smooth display)
		usersDiv.appendChild(fragment);
	
		console.log('All users rendered successfully.');
	});
	
	// Function to load images asynchronously
	function loadsearchedImageAsync(src) {
		return new Promise((resolve, reject) => {
			if (!src) {
