...
document.addEventListener('DOMContentLoaded', () => {
	...
	socket.on('foundUsers', async (founded) => {
		...
		founded.forEach((user) => {
			const userDiv = document.createElement('div');
			userDiv.classList.add('user');
			userDiv.style.visibility = 'hidden'; // Hide initially without affecting layout
		...
		// Set visibility to 'visible' once all images are loaded
		document.querySelectorAll('.user').forEach(user => {
			user.style.visibility = 'visible';  // Make the user div visible now that images are loaded
		});
	});
