...
document.addEventListener('DOMContentLoaded', () => {
	...
	socket.on('foundUsers', async (founded) => {
		...
		// Loop through the users and create the user divs
		founded.forEach((user) => {
			const userDiv = document.createElement('div');
			userDiv.classList.add('user');
			userDiv.style.display = 'none'; // Hide initially to prevent layout shift
			...
			// Load profile image
			const imageLoadPromise = updateSearchedProfileImage(profileContainer, user.profileImage, initials);
			imageLoadPromises.push(imageLoadPromise);
	
			// Track image loading
			imageLoadPromise.then(() => {
				imagesLoaded++;
				if (imagesLoaded === founded.length) {
					// All images loaded, show users
					document.querySelectorAll('.user').forEach(user => {
						user.style.display = 'flex'; // Reveal users after all images are loaded
					});
				}
			});
		});
	...
	async function updateSearchedProfileImage(container, imageSrc, initials) {
		...
		container.appendChild(placeholder); // Show a placeholder
		...
		try {
			...
			img.style.borderRadius = '50%';
			img.classList.add('userAvatar');
			container.appendChild(img); // Add the loaded image
			initials.style.display = 'none'; // Hide initials
			placeholder.remove(); // Remove placeholder once image is loaded
		} catch (error) {
			console.error(error.message);
			initials.style.display = 'flex'; // Show initials if image loading fails
			placeholder.remove(); // Remove placeholder if image fails
		}
	}
