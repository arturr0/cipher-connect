...
document.addEventListener('DOMContentLoaded', () => {
	...
	socket.on('foundUsers', async (founded) => {
		...
		founded.forEach((user) => {
			...
			// Load profile image (add lazy loading)
	...
	// Function to load the image asynchronously with lazy loading
	...
	// Function to update profile image with lazy loading
	async function updateSearchedProfileImage(container, imageSrc, initials) {
		if (!imageSrc) {
			initials.style.display = 'flex';
			return;
		...
		try {
			...
			// Add an IntersectionObserver to handle lazy loading
